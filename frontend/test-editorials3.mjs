...
async function run() {
  const { data, error } = await supabase.from('editorials').select('id, headline, image_url, status');
  console.log('Error:', error);
  console.log('Editorials:', JSON.stringify(data, null, 2));

  const { data: data2, error: error2 } = await supabase.from('articles').select('id, headline, image_url, status').eq('category', 'Editorial');
  console.log('Articles Error:', error2);
  console.log('Articles:', JSON.stringify(data2, null, 2));
}
