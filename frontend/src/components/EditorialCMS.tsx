...
export default function EditorialCMS({ onClose, session }: EditorialCMSProps) {
  ...
  const fetchData = () => {
    ...
    authFetch("/api/admin/editorials")
      ...
      .then(data => {
        ...
        const userEditorials = data
          .map((e: any) => ({ ...e, isEditorial: true }))
          .filter((e: any) => e.sourceName === userIdentifier);
          ...
          <div className="mb-8 pb-8 border-b border-zinc-200 dark:border-zinc-800">
            <div className="px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
              <p className="text-xs font-mono font-bold text-zinc-900 dark:text-white truncate">
                {session?.user?.user_metadata?.name || "Editorial User"}
              </p>
